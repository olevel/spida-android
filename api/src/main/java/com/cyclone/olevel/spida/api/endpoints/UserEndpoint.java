...
import com.cyclone.olevel.spida.api.model.User;
...
public class UserEndpoint {
    ...
    public User get(@Named("mId") Long mId) throws NotFoundException {
        logger.info("Getting User with ID: " + mId);
        User user = ofy().load().type(User.class).id(mId).now();
        if (user == null) {
    ...
    public User update(@Named("mId") Long mId, User user) throws NotFoundException {
